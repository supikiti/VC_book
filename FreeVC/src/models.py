...
class SynthesizerTrn(nn.Module):
  ...
  def forward(
    ...
    if c_lengths == None:
      c_lengths = (
          torch.ones(c.size(0)) * c.size(-1)
        ).to(c.device)
    if spec_lengths == None:
      spec_lengths = (
          torch.ones(spec.size(0)) * spec.size(-1)
        ).to(spec.device)
    ...
    # 線形スペクトログラムを事後エンコーダに通す
    z, m_q, logs_q, spec_mask = self.enc_q(spec, spec_lengths, g=g) 

    # 話者埋め込みを条件付きとし、正規化フローを通す
    z_p = self.flow(z, spec_mask, g=g)

    # 計算量削減のため、ランダムに時間方向でsegment sizeで切り出し
