...
def main(args):
    ...
    max_context_window = sr // hop_length * 30
    overlap_wave_len = overlap_frame_len * hop_length
    description = ("ゼロショット音声変換（in-context learning）。"
                   "参考音声が25秒を超える場合は自動的にカットされます。<br>"
                   "ソース音声と参考音声の合計が30秒を超える場合、ソース音声はチャンク単位で処理されます。")
    inputs = [
        gr.Audio(type="filepath", label="ソース音声"),
        gr.Audio(type="filepath", label="参考音声（変換先の声）"),
        gr.Slider(minimum=1, maximum=200, value=10, step=1, label="拡散ステップ数", info="デフォルト10。50〜100で最高品質"),
        gr.Slider(minimum=0.5, maximum=2.0, step=0.1, value=1.0, label="長さ調整", info="1.0未満で速く、1.0超で遅く"),
        gr.Slider(minimum=0.0, maximum=1.0, step=0.1, value=0.7, label="推論CFGレート", info="微小な影響あり"),
        gr.Checkbox(label="F0自動調整", value=True,
                    info="変換先の声に合わせてF0を大まかに調整します"),
        gr.Slider(label='ピッチシフト', minimum=-24, maximum=24, step=1, value=0, info="半音単位のピッチ変換"),
    ]

    outputs = [gr.Audio(label="ストリーミング出力", streaming=True, format='mp3'),
               gr.Audio(label="完全な出力音声", streaming=False, format='wav')]
