...
from src.mel_processing import spectrogram_torch, spec_to_mel_torch
from src.utils import load_wav_to_torch, load_filepaths_and_text, transform
#import h5py
...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, audiopaths, hparams):
        ...
        self.wav_dir = hparams.data.wav_dir
        self.spk_dir = hparams.data.spk_dir
        self.wavlm_dir = hparams.data.wavlm_dir
        self.sr_wavlm_dir = hparams.data.sr_wavlm_dir
    ...
    def _filter(self):
        ...
        for audiopath in self.audiopaths:
            wav_path = os.path.join(self.wav_dir, audiopath[0])
            lengths.append(os.path.getsize(wav_path) // (2 * self.hop_length))
        self.lengths = lengths

    def get_audio(self, rel_path):
        # wavファイルの読み込み
        wav_path = os.path.join(self.wav_dir, rel_path)
        ...
        spec_filename = wav_path.replace(".wav", ".spec.pt")
        ...
        # 話者埋め込みの取得
        if self.use_spk:
            spk_filename = os.path.join(
                self.spk_dir, rel_path.replace(".wav", ".npy"))
            spk = torch.from_numpy(np.load(spk_filename))

        # SSL特徴量の取得
        if not self.use_sr:
            c_filename = os.path.join(
                self.wavlm_dir, rel_path.replace(".wav", ".pt"))
            c = torch.load(c_filename).squeeze(0)
        else:
            i = random.randint(68, 92)
            c_filename = os.path.join(
                self.sr_wavlm_dir, rel_path.replace(".wav", f"_{i}.pt"))
            c = torch.load(c_filename).squeeze(0)
